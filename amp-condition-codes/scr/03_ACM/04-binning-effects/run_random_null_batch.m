...
base_result_dir = 'F:\PhD Code\My_PhD_Project\results\Modulated_FC_RandomNull\';
...
for i = 1:length(subjects)
    ...
    subj_metrics = calc_random_null_metrics(ts, 'n_levels', 20, ...
        'req_metrics', {'all'}); 
    
    % --- Save Results ---
    [~, name_no_ext, ~] = fileparts(subj_name);
    % Added '_dyn_mod' suffix to easily identify the output files
    save_path = fullfile(output_dir, [name_no_ext '_random_null.mat']);
