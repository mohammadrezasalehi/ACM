...
bin_strategy = 'SlidingWindow'; 
folder_suffix = sprintf('Strategy_%s', bin_strategy);
file_suffix = sprintf('_dyn_mod_%s.mat', bin_strategy);

% Base directory for alternatives
base_dir = 'F:\PhD Code\My_PhD_Project\results\Modulated_FC_Alternatives\';
input_dir = fullfile(base_dir, folder_suffix, session, clean_pipe);

% Output Group Directory
group_out_dir = fullfile(base_dir, folder_suffix, 'Group_Level', session);
if ~exist(group_out_dir, 'dir'), mkdir(group_out_dir); end

% Get Subject List
files = dir(fullfile(input_dir, ['*' file_suffix]));
