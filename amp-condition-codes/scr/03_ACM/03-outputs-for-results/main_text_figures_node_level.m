...
% this finalize the pictures for using in paper
% use all sessions in this code
...
addpath('F:\PhD Code\My_PhD_Project\src\04_utility');
...
fprintf('Loading Group-Level matrices for All Session and Mean Them ...\n');
var_mat = 0;
t_mat = 0;
zcr_mat = 0;
poly_prev = 0;
poly_mode = 0;
for i = 1:4
    group_dir = fullfile(project_root, 'results', 'Modulated_FC', 'Group_Level', session{i});
    
    V = load(fullfile(group_dir, 'Group_Variance_Consensus.mat'));
    var_mat = var_mat + V.Consensus_Percent;
    T = load(fullfile(group_dir, 'Group_Slope_Tmap.mat'));         
    t_mat = t_mat + T.T_map;
    Z = load(fullfile(group_dir, 'Group_ZCR_Median.mat'));         
    zcr_mat = zcr_mat + Z.Median;
    P = load(fullfile(group_dir, 'Group_Poly_Prevalence.mat'));    
    poly_prev = poly_prev + P.Prevalence_Percent;
    poly_mode = poly_mode + P.Mode;
end
var_mat = var_mat/4;
t_mat = t_mat/4;
zcr_mat = zcr_mat/4;
poly_prev = poly_prev/4;
poly_mode = poly_mode/4;
...
%% 3.5. Export Node-Level Features to CSV and Text (Top-10 Analysis)
fprintf('Exporting Node Features and Top-10 Analysis...\n');

export_dir = fullfile(project_root, 'results', 'Modulated_FC', 'Group_Level');

% 1. Create a comprehensive Table for all 200 nodes
NodeTable = table();
NodeTable.NodeID = (1:n_rois)';
NodeTable.RegionName = roi_names'; % From atlas info
NodeTable.NetworkName = roi_nets'; % From atlas info

NodeTable.Var_Mean       = NodeFeatures.Var_Mean;
NodeTable.Var_Entropy    = NodeFeatures.Var_Entropy;
NodeTable.Slope_Facil    = NodeFeatures.Slope_Facil;
NodeTable.Slope_Inhib    = NodeFeatures.Slope_Inhib;
NodeTable.Slope_Polarity = NodeFeatures.Slope_Polarity;
NodeTable.ZCR_Mean       = NodeFeatures.ZCR_Mean;
NodeTable.Poly_Footprint = NodeFeatures.Poly_Footprint;
NodeTable.Poly_Entropy   = NodeFeatures.Poly_Entropy;

% Save Full Table to CSV
csv_path = fullfile(export_dir, 'Node_Level_Features_All200.csv');
writetable(NodeTable, csv_path);
fprintf('Full feature table saved to: %s\n', csv_path);

% 2. Generate Top-10 Text Report
txt_path = fullfile(export_dir, 'Top10_Nodes_Report.txt');
fid = fopen(txt_path, 'w');
fprintf(fid, '==========================================================\n');
fprintf(fid, 'TOP-10 NODES FOR EACH MODULATORY METRIC\n');
fprintf(fid, '==========================================================\n\n');

% List of metrics to analyze for Top-10
metrics_to_rank = {'Var_Mean', 'Var_Entropy', 'Slope_Facil', 'Slope_Inhib', ...
                   'Slope_Polarity', 'ZCR_Mean', 'Poly_Footprint'};
               
metric_titles = {'Highest Global Modulatory Strength (Var_Mean)', ...
                 'Highest Modulatory Diversity (Var_Entropy)', ...
                 'Strongest Facilitators (Slope_Facil)', ...
                 'Strongest Inhibitors (Slope_Inhib)', ...
                 'Most Extreme Facilitatory Polarity (Slope_Polarity -> +1)', ...
                 'Highest Global Volatility (ZCR_Mean)', ...
                 'Largest Non-Linear Footprint (Poly_Footprint)'};

for m = 1:length(metrics_to_rank)
    metric = metrics_to_rank{m};
    vals = NodeFeatures.(metric);
    
    % Sort in descending order
    [sorted_vals, sort_idx] = sort(vals, 'descend');
    
    fprintf(fid, '--- %s ---\n', metric_titles{m});
    fprintf(fid, 'Rank\tValue\t\tNetwork\t\t\tRegion Name\n');
    fprintf(fid, '----------------------------------------------------------\n');
    
    for k = 1:10
        idx = sort_idx(k);
        fprintf(fid, '%d\t%.4f\t\t%-15s\t\t%s\n', k, sorted_vals(k), roi_nets{idx}, roi_names{idx});
    end
    fprintf(fid, '\n');
    
    % Special case for Polarity: Let's also get the Top 10 Extreme Inhibitors (Closest to -1)
    if strcmp(metric, 'Slope_Polarity')
        [sorted_vals_asc, sort_idx_asc] = sort(vals, 'ascend');
        fprintf(fid, '--- Most Extreme Inhibitory Polarity (Slope_Polarity -> -1) ---\n');
        fprintf(fid, 'Rank\tValue\t\tNetwork\t\t\tRegion Name\n');
        fprintf(fid, '----------------------------------------------------------\n');
        for k = 1:10
            idx = sort_idx_asc(k);
            fprintf(fid, '%d\t%.4f\t\t%-15s\t\t%s\n', k, sorted_vals_asc(k), roi_nets{idx}, roi_names{idx});
        end
        fprintf(fid, '\n');
    end
    
    % Special case for mean ofvariance: Let's also get the Top 10 LOWEST variance (Targeted Specialists)
    if strcmp(metric, 'Var_Mean')
        [sorted_vals_asc, sort_idx_asc] = sort(vals, 'ascend');
        fprintf(fid, '--- Lowest Global Modulatory Strength ---\n');
        fprintf(fid, 'Rank\tValue\t\tNetwork\t\t\tRegion Name\n');
        fprintf(fid, '----------------------------------------------------------\n');
        for k = 1:10
            idx = sort_idx_asc(k);
    ...
    % Special case for Entropy: Let's also get the Top 10 LOWEST entropy (Targeted Specialists)
    if strcmp(metric, 'Var_Entropy')
        [sorted_vals_asc, sort_idx_asc] = sort(vals, 'ascend');
        fprintf(fid, '--- Lowest Modulatory Diversity / Targeted Specialists (Var_Entropy) ---\n');
        fprintf(fid, 'Rank\tValue\t\tNetwork\t\t\tRegion Name\n');
        fprintf(fid, '----------------------------------------------------------\n');
        for k = 1:10
            idx = sort_idx_asc(k);
...
fclose(fid);
fprintf('Top-10 Text Report saved to: %s\n\n', txt_path);

%% 4. Visualization on 3D Cortical Surface
% We will create 1 figure per variable, containing multiple rows (one for each metric)

% --- Figure 1: Variance ---
fig1 = figure('Name', 'Node-Level Variance Maps', 'Position', [50, 50, 1200, 500], 'Color', 'w');
sgtitle('Variance Modulatory Features (Consensus Map)', 'FontSize', 18, 'FontWeight', 'bold');
plot_row_on_surf(1, 2, NodeFeatures.Var_Mean, sr, sl, gr, gl,  {'Global Modulatory', 'Strength (Mean)'}, turbo);
plot_row_on_surf(2, 2, NodeFeatures.Var_Entropy, sr, sl, gr, gl, {'Modulatory Diversity', '(Shannon Entropy)'}, turbo);
...
sgtitle('Slope / Directional Modulatory Features', 'FontSize', 18, 'FontWeight', 'bold');

% Apply Log10(x+1) transformation for visualization
log_facil = (NodeFeatures.Slope_Facil);
log_inhib = (NodeFeatures.Slope_Inhib);
...
plot_row_on_surf(1, 3, log_facil, sr, sl, gr, gl, {'Facilitator Index'}, turbo);
plot_row_on_surf(2, 3, log_inhib, sr, sl, gr, gl, {'Inhibitor Index'}, turbo);
